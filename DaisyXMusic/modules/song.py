...
def song(client, message):
    ...
    try:
        ...
        m.edit("❌ Heç nə tapılmadı.\n\nBaşqa musiqi axtarın və ya yenidən cəhd edin.")
        print(str(e))
        return
    m.edit("Yüklənir♻️ ")
    try:
...
async def jssong(_, message):
    ...
    if is_downloading:
        await message.reply_text(
            "Başqa bir yükləmə davam edir, az sonra yenidən cəhd edin."
        )
        return
    is_downloading = True
    ...
    try:
        ...
        with youtube_dl.YoutubeDL(opts) as ytdl:
            ...
            if duration > DURATION_LIMIT:
                await pablo.edit(
                    f"❌ {DURATION_LIMIT} dəqiqədən böyük fayllara icazə verilmir, bu fayl {duration} dəqiqədir"
                )
                is_downloading = False
                return
